class DerivedValuesController < ApplicationController
  ...
  def get_json_data(datavalue)
    @sites = Site.where(siteid: api_params[:siteids])
    dv = datavalue.model.order(utcdatetime: :asc)
    if api_params[:startdate].present?
      dv = dv.startdate(Date.parse(api_params[:startdate]).beginning_of_day)
    end
    if api_params[:enddate].present?
      dv = dv.enddate(Date.parse(api_params[:enddate]).end_of_day)
    end
    
    @sites.collect do |site|
      {
        name: site.sitename,
        data: dv.where(siteid: site.siteid).collect do |dv|
